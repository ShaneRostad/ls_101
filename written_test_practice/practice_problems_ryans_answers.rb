...
On lin 4 we are calling the `.each` method on `array` and passing in the block spanning lines 3-5 as an argument. `.each` iterates through each element of the array and variable `element` is assigned a reference to the value of said element. On line 5 variable `a` is re-assigned the value of `element`. Since `a` was initialized in main scope, it is accessible here in the blocks inner scope, and therefore can be re-assigned. On the first iteration, it is assigned a value of `1`, then `2`, then `3`. Since the last iteration is the last line of code run in the block, the return value within the block is `3`, however, `.each` ignores the return value and instead returns the original collection `[1, 2, 3]`.
